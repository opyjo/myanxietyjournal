...
function mapDailyCheckin(id: string, data: FirebaseFirestore.DocumentData): DailyCheckin {
  return {
    id,
    ...
    medicationStatuses: Array.isArray(data.medicationStatuses)
      ? data.medicationStatuses.map((item: Record<string, unknown>) => ({
...
function mapTriggerLog(id: string, data: FirebaseFirestore.DocumentData): TriggerLog {
  return {
    id,
...
function mapMedicationItem(id: string, data: FirebaseFirestore.DocumentData): MedicationItem {
  return {
    id,
...
function createFingerprint(bundle: AnalysisSourceBundle) {
  return createHash("sha256").update(JSON.stringify(bundle)).digest("hex");
}

async function loadSourceBundle(uid: string, rangeStart: string, rangeEnd: string) {
  ...
  const medications = medicationsSnapshot.docs.map((doc) =>
    mapMedicationItem(doc.id, doc.data()),
  );
...
function getAnthropicClient() {
  const apiKey =
    anthropicApiKey.value() || process.env.ANTHROPIC_API_KEY || process.env.ANTHROPIC_KEY;

  if (!apiKey) {
    throw new HttpsError(
      "failed-precondition",
      "Anthropic API key is not configured for this project.",
    );
  }
...
async function generateAnalysis(sourceBundle: AnalysisSourceBundle) {
...
interface AnalyzePatternDependencies {
  loadSourceBundle: typeof loadSourceBundle;
  findCachedAnalysis: typeof findCachedAnalysis;
  generateAnalysis: typeof generateAnalysis;
  createFingerprint: typeof createFingerprint;
  saveAnalysisRun: (args: {
    uid: string;
    rangeStart: string;
    rangeEnd: string;
    sourceBundle: AnalysisSourceBundle;
    sourceFingerprint: string;
...
export async function analyzePatternsCore(
  {
    uid,
    rangeStart,
    rangeEnd,
  ...
  dependencies: Partial<AnalyzePatternDependencies> = {},
) {
  const deps: AnalyzePatternDependencies = {
    loadSourceBundle,
    findCachedAnalysis,
    generateAnalysis,
    createFingerprint,
  ...
  if (sourceBundle.checkins.length < MIN_ANALYSIS_CHECKINS) {
    throw new HttpsError(
      "failed-precondition",
      `At least ${MIN_ANALYSIS_CHECKINS} check-ins are required for analysis.`,
    );
  }

  const sourceFingerprint = deps.createFingerprint(sourceBundle);
  const cached = await deps.findCachedAnalysis(
    uid,
    rangeStart,
    rangeEnd,
    sourceFingerprint,
